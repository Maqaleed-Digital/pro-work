...
const { canCreateDeliveryArtifact, requiredWorkItemStatus, initialEvidenceState, initialReviewState } = require("./authz");
...
function getDeliveryArtifactById(deliveryArtifactId) {
  ...
  return ok("DELIVERY_ARTIFACT_DETAIL_FETCHED", { item, updatedAt: state.updatedAt });
}
...
function advanceOpportunityStage(opportunityId, input, headers) {
  ...
  writeState(state);
  return ok("OPPORTUNITY_STAGE_ADVANCED", { item: opp, boardQueueCount: state.opportunities.filter((i) => i.stage === "BOARD_REVIEW").length, eventCount: state.events.length });
}
...
function approveOpportunity(opportunityId, input, headers) {
  ...
  opp.stage = "APPROVED";
  opp.updatedAt = nowIso();
  opp.finalDecision = { approvalId: approval.approvalId, decisionType: "APPROVE", actorId, actorRole, reason, at: nowIso() };
  ...
  writeState(state);
  return ok("OPPORTUNITY_APPROVED", { item: opp, approval, boardQueueCount: state.opportunities.filter((i) => i.stage === "BOARD_REVIEW").length, eventCount: state.events.length });
}
...
function createWorkItem(opportunityId, input, headers) {
  ...
  writeState(state);
  return created("WORK_ITEM_CREATED", { item: workItem, executionQueueCount: executionQueueCount(state), eventCount: state.events.length });
}
...
function transitionWorkItem(workItemId, headers, action, toStatus) {
  ...
  writeState(state);
  return ok("WORK_ITEM_TRANSITION_EXECUTED", { item, transition, executionQueueCount: executionQueueCount(state), eventCount: state.events.length });
}
...
function createDeliveryArtifact(workItemId, input, headers) {
  ...
  const artifactType = String((input && input.artifactType) || "").trim();
  const state = readState();
  const workItem = state.workItems.find((i) => i.workItemId === workItemId);

  if (!workItem) return rejected(404, "WORK_ITEM_NOT_FOUND", [{ field: "workItemId", message: "Work item not found" }]);

  if (workItem.status !== requiredWorkItemStatus()) {
    return rejected(422, "DELIVERY_ARTIFACT_INVALID", [{ field: "status", message: `Delivery artifact creation requires work item status ${requiredWorkItemStatus()}, current: ${workItem.status}` }]);
  }

  if (!canCreateDeliveryArtifact(actorRole)) {
    return rejected(403, "DELIVERY_ARTIFACT_FORBIDDEN", [{ field: "x-actor-role", message: "Only board_operator may create delivery artifacts" }]);
  }
  ...
  if (!artifactType) missing.push("artifactType");
  if (missing.length > 0) return rejected(422, "DELIVERY_ARTIFACT_INVALID", [{ field: "payload", message: "Missing required fields", fields: missing }]);
  ...
  writeState(state);
  return created("DELIVERY_ARTIFACT_CREATED", { item: deliveryArtifact, deliveryArtifactCount: state.deliveryArtifacts.length, eventCount: state.events.length });
}
