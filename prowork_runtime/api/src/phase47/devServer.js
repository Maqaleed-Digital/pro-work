...
const PORT = Number(process.env.PHASE47_PORT || "43147");
const DEMO_DIR = path.resolve(__dirname, "../../../web/phase47_demo");
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/work-items") {
    return send(res, getAllWorkItems());
  }

  if (req.method === "GET" && url.pathname === "/api/execution/queue") {
    return send(res, getExecutionQueue());
  }
  ...
  if (req.method === "GET" && /^\/api\/work-items\/[^/]+$/.test(url.pathname)) {
    const workItemId = url.pathname.split("/").pop();
    return send(res, getWorkItemById(workItemId));
  }

  // GET /api/opportunities/:id/work-items
  if (req.method === "GET" && /^\/api\/opportunities\/[^/]+\/work-items$/.test(url.pathname)) {
    const parts = url.pathname.split("/");
    const opportunityId = parts[3];
    return send(res, getWorkItemsByOpportunity(opportunityId));
  }
  ...
  if (req.method === "POST" && /^\/api\/opportunities\/[^/]+\/work-items$/.test(url.pathname)) {
    const parts = url.pathname.split("/");
    const opportunityId = parts[3];
    try {
      const body = await readJson(req);
      return send(res, createWorkItem(opportunityId, body, req.headers));
    } catch (error) {
  ...
  if (req.method === "GET" && url.pathname === "/phase47-demo") {
    return serveFile(res, "index.html", "text/html; charset=utf-8");
  }

  if (req.method === "GET" && url.pathname === "/phase47-demo/app.js") {
    return serveFile(res, "app.js", "application/javascript; charset=utf-8");
  }

  if (req.method === "GET" && url.pathname === "/phase47-demo/styles.css") {
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`PHASE47_SERVER_READY http://127.0.0.1:${PORT}`);
});
