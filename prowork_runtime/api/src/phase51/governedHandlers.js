...
const { canCreateCertification, initialCertificationState, initialAuditExportState } = require("./authz");
...
function getCommandCenterState() {
  ...
  return ok("COMMAND_CENTER_STATE_FETCHED", {
    summary: {
      ...
      certificationCount: state.certifications.length,
    ...
    latestCertification: state.certifications[state.certifications.length - 1] || null,
...
function getDeliveryArtifactsForWorkItem(workItemId) {
  const state = readState();
  const wi = state.workItems.find((i) => i.workItemId === workItemId);
  if (!wi) return rejected(404, "WORK_ITEM_NOT_FOUND", [{ field: "workItemId", message: "Work item not found" }]);
...
function getEvidencePackById(evidencePackId) {
  ...
  const certifications = state.certifications.filter((i) => i.evidencePackId === evidencePackId);
  return ok("EVIDENCE_PACK_DETAIL_FETCHED", { item, certifications, updatedAt: state.updatedAt });
}

function getCertificationsForEvidencePack(evidencePackId) {
  const state = readState();
  const ep = state.evidencePacks.find((i) => i.evidencePackId === evidencePackId);
  if (!ep) return rejected(404, "EVIDENCE_PACK_NOT_FOUND", [{ field: "evidencePackId", message: "Evidence pack not found" }]);
  const items = state.certifications.filter((i) => i.evidencePackId === evidencePackId);
  return ok("CERTIFICATION_LIST_FETCHED", { evidencePackId, items, count: items.length, updatedAt: state.updatedAt });
}

function getCertifications() {
  const state = readState();
  return ok("CERTIFICATION_LIST_FETCHED", { items: state.certifications, count: state.certifications.length, updatedAt: state.updatedAt });
}

function getCertificationById(certificationId) {
  const state = readState();
  const item = state.certifications.find((i) => i.certificationId === certificationId);
  if (!item) return rejected(404, "CERTIFICATION_NOT_FOUND", [{ field: "certificationId", message: "Certification not found" }]);
  return ok("CERTIFICATION_DETAIL_FETCHED", { item, updatedAt: state.updatedAt });
}

function getAuditExportByCertificationId(certificationId) {
  const state = readState();
  const certification = state.certifications.find((i) => i.certificationId === certificationId);
  if (!certification) return rejected(404, "CERTIFICATION_NOT_FOUND", [{ field: "certificationId", message: "Certification not found" }]);
  ...
  return ok("AUDIT_EXPORT_FETCHED", { certification, evidencePack, deliveryArtifact, workItem, opportunity, generatedAt: nowIso() });
}
...
function createIntake(input) {
  ...
  writeState(state);
  return created("INTAKE_CREATED", { intake, opportunity, eventCount: state.events.length });
}
...
function transitionWorkItem(workItemId, headers, action, toStatus) {
  ...
  appendEvent(state, makeEvent({ eventType: action === "start" ? "WORK_ITEM_STARTED" : "WORK_ITEM_COMPLETED", aggregateType: "WORK_ITEM", aggregateId: workItemId, actorId, actorRole, payload: { workItemId, fromStatus, toStatus } }));
  ...
  appendEvent(state, makeEvent({ eventType: "EXECUTION_PROGRESS_UPDATED", aggregateType: "COMMAND_STATE", aggregateId: "GLOBAL", actorId, actorRole, payload: { workItemId, completedWorkItemCount: state.workItems.filter((i) => i.status === "COMPLETED").length } }));
  writeState(state);
...
function createDeliveryArtifact(workItemId, input, headers) {
  ...
  const deliveryArtifact = { deliveryArtifactId: id("delivery"), workItemId, opportunityId: workItem.opportunityId, title: String((input && input.title) || "").trim(), summary: String((input && input.summary) || "").trim(), artifactType: String((input && input.artifactType) || "").trim(), evidenceState: "EVIDENCE_CAPTURED", reviewState: "DELIVERY_VISIBLE", actorId, actorRole, createdAt: nowIso(), updatedAt: nowIso() };
  ...
  appendEvent(state, makeEvent({ eventType: "COMMAND_CENTER_DELIVERY_UPDATED", aggregateType: "COMMAND_STATE", aggregateId: "GLOBAL", actorId, actorRole, payload: { deliveryArtifactCount: state.deliveryArtifacts.length } }));
  writeState(state);
...
function createEvidencePack(deliveryArtifactId, input, headers) {
  ...
  const da = state.deliveryArtifacts.find((i) => i.deliveryArtifactId === deliveryArtifactId);
  if (!da) return rejected(404, "DELIVERY_ARTIFACT_NOT_FOUND", [{ field: "deliveryArtifactId", message: "Delivery artifact not found" }]);
  if (actorRole !== "board_operator") return rejected(403, "EVIDENCE_PACK_FORBIDDEN", [{ field: "x-actor-role", message: "Only board_operator may create evidence packs" }]);
  const evidencePack = { evidencePackId: id("evidencepack"), deliveryArtifactId, workItemId: da.workItemId, opportunityId: da.opportunityId, title: String((input && input.title) || "").trim(), summary: String((input && input.summary) || "").trim(), packType: String((input && input.packType) || "").trim(), trustState: "TRUST_CAPTURED", exportState: "PACK_VISIBLE", actorId, actorRole, createdAt: nowIso(), updatedAt: nowIso() };
  ...
  writeState(state);
  return created("EVIDENCE_PACK_CREATED", { item: evidencePack, eventCount: state.events.length });
}

function createCertification(evidencePackId, input, headers) {
  const actorId = String(headers["x-actor-id"] || "").trim();
  const actorRole = String(headers["x-actor-role"] || "").trim();
  const title = String((input && input.title) || "").trim();
  const summary = String((input && input.summary) || "").trim();
  const certificationType = String((input && input.certificationType) || "").trim();
  const state = readState();
  const ep = state.evidencePacks.find((i) => i.evidencePackId === evidencePackId);
  if (!ep) return rejected(404, "EVIDENCE_PACK_NOT_FOUND", [{ field: "evidencePackId", message: "Evidence pack not found" }]);
  if (!canCreateCertification(actorRole)) return rejected(403, "CERTIFICATION_FORBIDDEN", [{ field: "x-actor-role", message: "Only board_operator may create certifications" }]);
  const missing = [];
  if (!actorId) missing.push("x-actor-id");
  if (!title) missing.push("title");
  if (!summary) missing.push("summary");
  if (!certificationType) missing.push("certificationType");
  if (missing.length > 0) return rejected(422, "CERTIFICATION_INVALID", [{ field: "payload", message: "Missing required fields", fields: missing }]);
  const certification = { certificationId: id("certification"), evidencePackId, deliveryArtifactId: ep.deliveryArtifactId, workItemId: ep.workItemId, opportunityId: ep.opportunityId, title, summary, certificationType, certificationState: initialCertificationState(), auditExportState: initialAuditExportState(), actorId, actorRole, createdAt: nowIso(), updatedAt: nowIso() };
  state.certifications.push(certification);
  appendEvent(state, makeEvent({ eventType: "CLOSURE_CERTIFICATION_CREATED", aggregateType: "CLOSURE_CERTIFICATION", aggregateId: certification.certificationId, actorId, actorRole, payload: { certificationId: certification.certificationId, evidencePackId, deliveryArtifactId: ep.deliveryArtifactId } }));
  appendEvent(state, makeEvent({ eventType: "AUDIT_EXPORT_GENERATED", aggregateType: "AUDIT_EXPORT", aggregateId: certification.certificationId, actorId, actorRole, payload: { certificationId: certification.certificationId, auditExportState: certification.auditExportState } }));
  appendEvent(state, makeEvent({ eventType: "COMMAND_CENTER_CERTIFICATION_UPDATED", aggregateType: "COMMAND_STATE", aggregateId: "GLOBAL", actorId, actorRole, payload: { certificationCount: state.certifications.length, evidencePackCount: state.evidencePacks.length } }));
  writeState(state);
  return created("CLOSURE_CERTIFICATION_CREATED", { item: certification, certificationCount: state.certifications.length, eventCount: state.events.length });
}
...
module.exports = {
  ...
  getCertificationsForEvidencePack, getCertifications, getCertificationById, getAuditExportByCertificationId,
  createIntake, advanceOpportunityStage, approveOpportunity, createWorkItem,
  startWorkItem, completeWorkItem, createDeliveryArtifact, createEvidencePack, createCertification
};
