...
const PORT = Number(process.env.PHASE45_PORT || "43145");
const DEMO_DIR = path.resolve(__dirname, "../../../web/phase45_demo");
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/phase45-demo") {
    return serveFile(res, "index.html", "text/html; charset=utf-8");
  }

  if (req.method === "GET" && url.pathname === "/phase45-demo/app.js") {
    return serveFile(res, "app.js", "application/javascript; charset=utf-8");
  }

  if (req.method === "GET" && url.pathname === "/phase45-demo/styles.css") {
  ...
  return sendText(res, 404, "application/json; charset=utf-8", JSON.stringify({
    ...
    code: "NOT_FOUND",
    data: null,
    errors: [{ message: "Route not found", path: url.pathname }],
    meta: {}
  }, null, 2));
});

server.listen(PORT, "127.0.0.1", () => {
  console.log(`PHASE45_SERVER_READY http://127.0.0.1:${PORT}`);
});
