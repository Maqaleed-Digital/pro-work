...
# Phase 52 evidence runner — mounts phase52Module into phase51 devServer for assurance routes
...
STATE_FILE="prowork_runtime/api/data/phase52-runtime.json"
EVIDENCE_DIR="${EVIDENCE_RUN_DIR:-evidence/phase52_$(date +%Y%m%dT%H%M%S)}"
...
# Write an inline server that combines Phase 51 handlers + Phase 52 module
SERVER_JS="$REPO_ROOT/.tmp_phase52_server.js"
cat > "$SERVER_JS" <<'NODESCRIPT'
const http = require("http");
const path = require("path");
const REPO_ROOT = process.env.PHASE52_REPO_ROOT;
const handlers = require(path.join(REPO_ROOT, "prowork_runtime/api/src/phase51/governedHandlers"));
const { readState } = require(path.join(REPO_ROOT, "prowork_runtime/api/src/phase51/governedStore"));
const { createPhase52Module } = require(path.join(REPO_ROOT, "prowork_runtime/api/src/phase52/phase52Module"));

const PORT = Number(process.env.PORT || "43152");

const phase52 = createPhase52Module({
  resolveState: () => {
    const s = readState();
    return {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (method === "GET" && url === "/health") {
      ...
      res.end(JSON.stringify({ ok: true, phase: 52, port: PORT }));
      return;
    }
    ...
    // Phase 52 assurance routes — check first
    const p52handled = await phase52.route(req, res, url);
    if (p52handled !== false) return;
...
server.listen(PORT, () => console.log(`Phase 52 server running on port ${PORT}`));
NODESCRIPT

log "Starting Phase 52 server on port $PORT"
PORT=$PORT PHASE52_REPO_ROOT=$REPO_ROOT node "$SERVER_JS" >> "$LOG" 2>&1 &
SERVER_PID=$!
...
# Full flow to populate state
log "--- Active: intake → approve → work item → complete → delivery → evidence pack → certification ---"
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/intake" \
  -H "content-type: application/json" \
  -d '{"tenantId":"tenant_001","requesterId":"req_001","title":"Phase 52 Board Assurance","summary":"Board assurance dashboard integration test for portfolio closure visibility"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/opportunities/$OPP_ID/approve" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"reason":"Approved for board assurance"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/opportunities/$OPP_ID/work-items" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Assurance work item","summary":"Board assurance work item execution"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/work-items/$WI_ID/delivery-artifacts" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Assurance artifact","summary":"Board assurance delivery artifact","artifactType":"BOARD_PACK"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/delivery-artifacts/$DA_ID/evidence-packs" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Assurance evidence pack","summary":"Board assurance evidence pack","packType":"BOARD_ASSURANCE"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/evidence-packs/$EP_ID/certifications" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Board assurance certification","summary":"Governed closure certification for board assurance","certificationType":"BOARD_ASSURANCE"}')
...
# Phase 52 board assurance routes
log "--- Board assurance routes ---"
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" "http://localhost:$PORT/api/board/assurance")
assert_status "board_assurance" 200 "$STATUS"
cp "$TMPBODY" "$EVIDENCE_DIR/board_assurance.json"
CERT_TOTAL=$(cat "$TMPBODY" | jq_node "d.data.totals.certifications")
[ "$CERT_TOTAL" -ge 1 ] || fail "certifications must be >= 1 in assurance totals, got $CERT_TOTAL"
log "OK certifications in assurance totals: $CERT_TOTAL"

STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" "http://localhost:$PORT/api/board/assurance/$OPP_ID")
assert_status "board_assurance_by_opp" 200 "$STATUS"
cp "$TMPBODY" "$EVIDENCE_DIR/board_assurance_opp.json"

STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" "http://localhost:$PORT/api/board/assurance/opp_nonexistent")
assert_status "board_assurance_404" 404 "$STATUS"

STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" "http://localhost:$PORT/api/board/portfolio/closure-summary")
assert_status "portfolio_closure_summary" 200 "$STATUS"
cp "$TMPBODY" "$EVIDENCE_DIR/portfolio_closure_summary.json"
