...
STATE_FILE="prowork_runtime/api/data/phase55-runtime.json"
EVIDENCE_DIR="${EVIDENCE_RUN_DIR:-evidence/phase55_$(date +%Y%m%dT%H%M%S)}"
mkdir -p "$EVIDENCE_DIR"
LOG="$EVIDENCE_DIR/run.log"
TMPBODY=$(mktemp)
SERVER_JS="$REPO_ROOT/.tmp_phase55_server.js"
...
const REPO = process.env.PHASE55_REPO_ROOT;
...
const PORT = Number(process.env.PORT || "43155");
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (method === "GET" && url === "/health") {
      ...
      res.end(JSON.stringify({ ok: true, phase: 55, port: PORT }));
      return;
    }
...
server.listen(PORT, () => console.log(`Phase 55 server running on port ${PORT}`));
NODESCRIPT

log "Starting Phase 55 server on port $PORT"
PORT=$PORT PHASE55_REPO_ROOT=$REPO_ROOT node "$SERVER_JS" >> "$LOG" 2>&1 &
SERVER_PID=$!
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/intake" \
  ...
  -d '{"tenantId":"tenant_001","requesterId":"req_001","title":"Phase 55 Recommendations","summary":"Governed board recommendation and advisory layer integration test"}')
assert_status "create_intake" 201 "$STATUS"

OPP_ID=$(cat "$TMPBODY" | jq_node "d.data.opportunity.opportunityId")
log "Opportunity: $OPP_ID"

# Recommendations before any work — should be ESCALATE_TO_BOARD (multiple gaps)
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" "http://localhost:$PORT/api/board/recommendations/$OPP_ID")
assert_status "recs_before_work" 200 "$STATUS"
REC_COUNT=$(cat "$TMPBODY" | jq_node "d.data.length")
[ "$REC_COUNT" -ge 3 ] || fail "Expected >= 3 recommendations before any work, got $REC_COUNT"
HAS_ESCALATE=$(cat "$TMPBODY" | jq_node "d.data.filter(r=>r.type==='ESCALATE_TO_BOARD').length")
[ "$HAS_ESCALATE" -ge 1 ] || fail "Expected ESCALATE_TO_BOARD recommendation, got $HAS_ESCALATE"
log "OK pre-work recommendations: $REC_COUNT (escalate=$HAS_ESCALATE)"
cp "$TMPBODY" "$EVIDENCE_DIR/recs_before_work.json"
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/opportunities/$OPP_ID/approve" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"reason":"Approved for recommendation test"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/opportunities/$OPP_ID/work-items" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Recommendation work item","summary":"Execute recommendation advisory work item"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/work-items/$WI_ID/delivery-artifacts" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Rec artifact","summary":"Recommendation delivery artifact","artifactType":"ADVISORY_PACK"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/delivery-artifacts/$DA_ID/evidence-packs" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Rec evidence pack","summary":"Recommendation evidence pack","packType":"ADVISORY_EVIDENCE"}')
...
STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" -X POST "http://localhost:$PORT/api/evidence-packs/$EP_ID/certifications" \
  -H "content-type: application/json" -H "x-actor-id: op1" -H "x-actor-role: board_operator" \
  -d '{"title":"Rec certification","summary":"Governed closure certification for recommendation","certificationType":"BOARD_ASSURANCE"}')
...
assert_status "recommendations_portfolio" 200 "$STATUS"
cp "$TMPBODY" "$EVIDENCE_DIR/recommendations_portfolio.json"
PORTFOLIO_COUNT=$(cat "$TMPBODY" | jq_node "d.data.length")
[ "$PORTFOLIO_COUNT" -ge 1 ] || fail "Expected >= 1 portfolio entry, got $PORTFOLIO_COUNT"
log "OK portfolio recommendation entries: $PORTFOLIO_COUNT"

STATUS=$(curl -s -w "%{http_code}" -o "$TMPBODY" "http://localhost:$PORT/api/board/recommendations/$OPP_ID")
assert_status "recommendations_detail" 200 "$STATUS"
cp "$TMPBODY" "$EVIDENCE_DIR/recommendations_detail.json"
FINAL_RECS=$(cat "$TMPBODY" | jq_node "d.data.length")
log "OK recommendations after full closure: $FINAL_RECS"
[ "$FINAL_RECS" -eq 0 ] || fail "Expected 0 recommendations after full closure, got $FINAL_RECS"
