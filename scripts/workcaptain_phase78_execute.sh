...
if [ "${#}" -ne 3 ]; then
  echo "usage: workcaptain_phase78_execute.sh <repo_root> <prior_evidence_dir> <output_evidence_dir>"
  exit 1
fi
...
python3 "${REPO_ROOT}/scripts/workcaptain_phase78_upgrade.py" \
  "${PRIOR_EVIDENCE_DIR}" \
  "${OUTPUT_EVIDENCE_DIR}" \
  "${REPO_ROOT}/config/intelligence/certification_upgrade_thresholds.json"

cat > "${OUTPUT_EVIDENCE_DIR}/EXECUTION_REPORT.md" <<REPORT
# PHASE 78 EXECUTION REPORT

- Repo root: ${REPO_ROOT}
- Prior evidence dir: ${PRIOR_EVIDENCE_DIR}
- Output evidence dir: ${OUTPUT_EVIDENCE_DIR}
- Phase 78 status: COMPLETE
- Upgrade mode: EVIDENCE_BACKED_REASSESSMENT
