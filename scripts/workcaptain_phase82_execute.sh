...
if [ "${#}" -ne 3 ]; then
  echo "usage: workcaptain_phase82_execute.sh <repo_root> <prior_evidence_dir> <output_evidence_dir>"
  exit 1
fi
...
python3 "${REPO_ROOT}/scripts/workcaptain_phase82_blocker_specific.py" \
  "${PRIOR_EVIDENCE_DIR}" \
  "${OUTPUT_EVIDENCE_DIR}" \
  "${REPO_ROOT}/config/intelligence/blocker_specific_validation_defaults.json"

cat > "${OUTPUT_EVIDENCE_DIR}/EXECUTION_REPORT.md" <<REPORT
# PHASE 82 EXECUTION REPORT

- Repo root: ${REPO_ROOT}
- Prior evidence dir: ${PRIOR_EVIDENCE_DIR}
- Output evidence dir: ${OUTPUT_EVIDENCE_DIR}
- Phase 82 status: COMPLETE
- Closure mode: BLOCKER_SPECIFIC
- No-guessing role: ENFORCED
- Certification rerun posture: BLOCKED_UNTIL_ALL_BLOCKERS_VALIDATED
REPORT
