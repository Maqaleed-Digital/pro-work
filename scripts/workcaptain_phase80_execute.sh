...
if [ "${#}" -ne 3 ]; then
  echo "usage: workcaptain_phase80_execute.sh <repo_root> <prior_evidence_dir> <output_evidence_dir>"
  exit 1
fi
...
python3 "${REPO_ROOT}/scripts/workcaptain_phase80_resolution.py" \
  "${PRIOR_EVIDENCE_DIR}" \
  "${OUTPUT_EVIDENCE_DIR}" \
  "${REPO_ROOT}/config/intelligence/blocker_resolution_defaults.json"

cat > "${OUTPUT_EVIDENCE_DIR}/EXECUTION_REPORT.md" <<REPORT
# PHASE 80 EXECUTION REPORT

- Repo root: ${REPO_ROOT}
- Prior evidence dir: ${PRIOR_EVIDENCE_DIR}
- Output evidence dir: ${OUTPUT_EVIDENCE_DIR}
- Phase 80 status: COMPLETE
- Resolution posture: HUMAN_APPROVAL_REQUIRED
- Certification rerun posture: BLOCKED_UNTIL_ALL_CLOSURE_EVIDENCE_VALIDATED
REPORT
