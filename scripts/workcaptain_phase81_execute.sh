...
if [ "${#}" -ne 3 ]; then
  echo "usage: workcaptain_phase81_execute.sh <repo_root> <prior_evidence_dir> <output_evidence_dir>"
  exit 1
fi
...
python3 "${REPO_ROOT}/scripts/workcaptain_phase81_closure.py" \
  "${PRIOR_EVIDENCE_DIR}" \
  "${OUTPUT_EVIDENCE_DIR}" \
  "${REPO_ROOT}/config/intelligence/blocker_closure_submission_defaults.json"

cat > "${OUTPUT_EVIDENCE_DIR}/EXECUTION_REPORT.md" <<REPORT
# PHASE 81 EXECUTION REPORT
...
- Closure posture: EVIDENCE_REQUIRED
